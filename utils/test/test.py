...
def test_model(env, model, render=False):
    ...
    num_rollouts = 5
    rewards = [[] for _ in range(num_rollouts)]
    np_rewards = np.zeros((num_rollouts, ))
    if render:
        env.connectUnity()
    for n_roll in range(num_rollouts):
        ...
        while not ( (ep_len >= max_ep_length)):
            ...
            rewards[n_roll].append(rew[0])
            #
            ep_len += 1
            # obs=obs["state"]
            
            #
            pos.append(obs["state"][0, 0:3].tolist())
            dpos.append(obs["state"][0, 6:9].tolist())
            euler.append(obs["state"][0, 3:6].tolist())
            deuler.append(obs["state"][0, 9:12].tolist())
            #
        ...
        ax_action0.step(t, actions[:, 0], color="C{0}".format(
            n_roll), label="trail: {0}".format(n_roll), linewidth=1)
        ...
        np_rewards[n_roll] = np.sum(rewards[n_roll])
        if np_rewards[n_roll] <=10:
            print("pos: ", pos[:,2])
    #
    if render:
        env.disconnectUnity()
