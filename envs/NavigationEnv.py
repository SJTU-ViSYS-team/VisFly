...
class NavigationEnv2(DroneGymEnvsBase):
    def __init__(
        ...
        self.target = th.tile(th.as_tensor([9, 0., 1] if target is None else target), (self.num_envs, 1))
        self.success_radius = 0.5
    ...
    def get_observation(
        ...
        dis_scale = (self.target - self.position).norm(dim=1).detach().clamp_min(self.max_sense_radius)
        state = th.hstack([
            (self.target - self.position) / dis_scale.unsqueeze(1),
            self.orientation,
            self.velocity / 10,
        ...
        return TensorDict({
            "state": state,
            "depth": 1 / (self.sensor_obs["depth"] + 1),
            # "target": self.target.to(self.device),
